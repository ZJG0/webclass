...
<div class="weizhi">
    <div class="weizhi_title">
        <span class="glyphicon glyphicon-volume-up"></span>已发通知
    </div>
</div>
...
<div class="container-fluid padTa">
    <table class="table gTable table-bordered">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td style="white-space: nowrap;overflow:hidden;text-overflow: ellipsis;"><?php echo ($vo["title"]); ?></td>
                <td><?php echo ($vo["date"]); ?></td>
                <td>
                    <span class="glyphicon glyphicon-edit span1" style="color:#c21f3c"><a href="<?php echo U('updataInform');?>?id=<?php echo ($vo["id"]); ?>" target="cont" target="cont">修改</a></span>
                </td>
                <td>
                    <span class="glyphicon glyphicon-remove span1" style="color:#c21f3c"><a onclick="return confirm('您确定要删除？')" href="<?php echo U('Admin/delInform');?>?id=<?php echo ($vo["id"]); ?>">删除</a></span>
                </td>
    ...
    <div class="row">
        <div class="col-sm-5 col-sm-offset-2" style="text-align: center">
            <ul class="pagination">
                <?php if($page > 1): ?><li><a href="<?php echo U('Admin/allInform');?>?page=<?php echo ($page-1); ?>">&laquo;</a></li><?php endif; ?>
                <?php $__FOR_START_32766__=0;$__FOR_END_32766__=$allPage;for($i=$__FOR_START_32766__;$i < $__FOR_END_32766__;$i+=1){ ?><li><a class="x<?php echo ($i+1); ?> "  href="<?php echo U('Admin/allInform');?>?page=<?php echo ($i+1); ?>"><?php echo ($i+1); ?></a></li><?php } ?>
                <?php if($page < $allPage): ?><li><a href="<?php echo U('Admin/allInform');?>?page=<?php echo ($page+1); ?>">&raquo;</a></li><?php endif; ?>
            </ul>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(".x<?php echo ($page); ?>").css('background','#ccc');
</script>
</html>